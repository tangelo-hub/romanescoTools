...
#' romanescoPlot(expression(plot(1:10)))
#' myPlotExpr <- expression({
#'    plot(1:10)
#'    abline(0, 1)
#' })
#' romanescoPlot(myPlotExpr)
#' 
...
#' 
#' # using lattice:
#' romanescoPlot(xyplot(rnorm(10) ~ rnorm(10)))
#' # pass lattice object
#' p <- dotplot(variety ~ yield | year * site, data=barley)
#' romanescoPlot(p)
#' 
#' # using ggplot
#' romanescoPlot(qplot(mpg, wt, data = mtcars))
#' # pass ggplot object
#' p <- qplot(mpg, wt, data = mtcars)
#' romanescoPlot(p)
#' @export
romanescoPlot <- function(plotObj, type = "png", width = 480, height = 480, res = 72, ...) {
   ...
   result <- base64enc:::base64encode(readBin(file, "raw", n = bytes))
   file.remove(file)
